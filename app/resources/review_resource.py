...
def create_review():
    ...
    try:
        ...
        if not data:
            return jsonify({"error": "Missing required fields", "message": "No data provided"}), 400
        ...
        for rating_field in optional_ratings:
            if rating_field in data and data[rating_field] is not None:
                if (
                    not isinstance(data[rating_field], int)
                    or data[rating_field] < 1
                    or data[rating_field] > 5
                ):
                    return (
                        jsonify(
                            {
...
def get_project_review(project_id):
    ...
    try:
        ...
        if not review:
            return (
                jsonify(
                    {"success": True, "review": None, "message": "No review found for this project"}
                ),
                200,
            )
        ...
        return jsonify({"error": "Failed to fetch review", "message": str(e)}), 500
            ...
            jsonify(
                {
                    ...
                    "statistics": {
                        ...
                        "average_communication": calculate_average_subrating(
                            reviews, "communication_rating"
                        ),
                        "average_quality": calculate_average_subrating(reviews, "quality_rating"),
                        "average_timeliness": calculate_average_subrating(
                            reviews, "timeliness_rating"
                        ),
                    },
                }
            ),
    ...
    try:
        ...
        if total_public_reviews > 0:
            avg_freelancer_rating = (
                sum(review.average_rating() for review in public_freelancer_reviews)
        ...
        traceback.print_exc()
        return jsonify({"error": "Failed to fetch user reviews", "message": str(e)}), 500
...
def delete_review(review_id):
    ...
    try:
        ...
        if review.client_id != current_user_id and (
...
def get_freelancer_review_stats(freelancer_id):
    ...
    try:
        ...
        return jsonify({"error": "Failed to fetch review statistics", "message": str(e)}), 500
...
def calculate_average_subrating(reviews, rating_field):
    ...
    ratings = [
        ...
        for review in reviews
        if getattr(review, rating_field) is not None
    ]
    if not ratings:
        return 0
