    ...
    try:
        ...
        if deliverable.cloudinary_public_id:
            CloudinaryService.delete_file(
                deliverable.cloudinary_public_id, resource_type="image"  # Adjust based on file_type
            )
...
def approve_deliverable(deliverable_id):
    ...
    try:
        ...
        if deliverable.status == "approved":
            return jsonify({"success": False, "error": "Deliverable already approved"}), 400
            ...
            if project.status == 'completed' and not project.is_sensitive:
                ...
                if not existing_portfolio:
                    ...
                    approved_deliverables = Deliverable.query.filter_by(
                        project_id=project.id,
                        status='approved'
                    ).order_by(Deliverable.version_number.desc()).all()
                    
                    # Use latest approved deliverable as cover
                    cover_deliverable = approved_deliverables[0] if approved_deliverables else deliverable
                    ...
                    portfolio_item = PortfolioItem(
                        ...
                        cover_image_url=cover_deliverable.thumbnail_url or cover_deliverable.file_url,
                        tags=tags,
                        is_visible=True,
                        is_featured=False,
                        display_order=0
                    ...
                    current_app.logger.info(f"Portfolio item already exists for project {project.id}")
...
def compare_versions():
    ...
    try:
        ...
        if not data or "version1_id" not in data or "version2_id" not in data:
            return jsonify({"success": False, "error": "Both version IDs are required"}), 400
        ...
        if version1.project_id != version2.project_id:
            return (
                jsonify({"success": False, "error": "Versions must be from the same project"}),
                400,
            )
