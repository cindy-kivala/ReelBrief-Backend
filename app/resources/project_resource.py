...
def create_project():
    ...
    for s in required_skills:
        ...
        link = ProjectSkill(
            skill=skill, required_proficiency=s.get("required_proficiency", "intermediate")
        )
...
def get_available_freelancers():
    ...
    try:
        available_freelancers = FreelancerProfile.query.filter_by(
            open_to_work=True, application_status="approved"
        ).all()
        ...
        return jsonify({"success": True, "freelancers": freelancers_data}), 200
...
def assign_freelancer(project_id):
    ...
    return (
        jsonify({"message": "Freelancer assigned successfully", "project": project.to_dict()}),
        200,
    )
