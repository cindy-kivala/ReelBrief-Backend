...
def test_tables_directly():
    ...
    database_url = os.getenv(
        "DATABASE_URL", "postgresql://reelbrief_user:cindy123@localhost/reelbrief_db"
    )
    ...
    try:
        ...
        """,
            (
                ...
                "First version with basic layout",
                "pending",
                current_time,
            ),
        )
        ...
        """,
            (
                deliverable_id,
                client_id,
                "Great work! The layout looks clean and modern.",
                "comment",
                "medium",
                False,
                current_time,
        ...
        """,
            (
                deliverable_id,
                client_id,
                "Can we make the header section larger?",
                "revision_request",
                "high",
                False,
                current_time,
...
def create_verified_users():
    ...
    try:
        ...
        cur.execute(
            "DELETE FROM users WHERE email IN ('test.client@example.com', 'freelancer@example.com')"
        )
