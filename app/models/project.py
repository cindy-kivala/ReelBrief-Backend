"""
Project Models - FIXED VERSION
Owner: Monica
Description: Project management with status tracking, deliverables, and skill requirements.
UPDATED: Fixed escrow_transaction relationship and to_dict method
"""
...
class Project(db.Model):
    ...
    assignment_requested = db.Column(db.Boolean, default=False)
    ...
    # Escrow transactions - FIXED: Use the correct relationship name
    ...
    def to_dict(self) -> dict:
        """Serialize for APIs - FIXED escrow access"""
        # Get the first escrow transaction if exists
        escrow_data = None
        if self.escrow_transactions:
            escrow = self.escrow_transactions[0]  # Get first escrow
            escrow_data = {
                "id": escrow.id,
        ...
        return {
            ...
            # FIXED: Use escrow_transactions (plural) instead of escrow_transaction (singular)
            "escrow": escrow_data,
        }
